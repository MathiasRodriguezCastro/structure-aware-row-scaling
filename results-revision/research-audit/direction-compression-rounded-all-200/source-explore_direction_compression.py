...
def transform(A,b,method,metadata):
    ...
    if method in ['Rounded-direction','Rational-direction','Rounded-all']:
        strategy={'Rounded-direction':'rounded','Rounded-all':'rounded-all','Rational-direction':'rational'}[method]
        return compress_matrix(A,b,strategy)
    if method=='Dyadic-GM':
...
def main():
    ...
    if args.saved_input:
        files=sorted(args.saved_input.glob('M*_s*.npz'))
        original_design=json.loads((args.saved_input/'design.json').read_text())
        blocks=original_design['blocks'];q=original_design['local_size'];local_rows=original_design.get('local_rows',2)
