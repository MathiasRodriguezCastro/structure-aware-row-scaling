...
def compile_block(A,b,eta=1e-6,epsilon=1e-6,all_guards=False):
    ...
    original_uncovered=[];intrinsic_uncovered=[];joint_proofs=[]
    for z in bad:
        gaps=[row_gap(A,b,z,lam,eta,eta) for lam in candidates]
        margins=[g-sum((lam[i]*eps/F(float(d[i])) for i in range(len(d))),F(0))
                 for lam,g in zip(candidates,gaps)]
        if max(margins)<=0:original_uncovered.append(z)
        else:
            i=int(np.argmax(margins));joint_proofs.append(dict(z=z,lambda_=list(map(str,candidates[i])),margin=str(margins[i])))
        # Intrinsic LP obstruction uses the exact original bounds, even though
        # the operational acceptance certificate allows bound tolerance eta.
        exact_box_gaps=[row_gap(A,b,z,lam,eta,0) for lam in candidates]
        if max(exact_box_gaps)<=0:intrinsic_uncovered.append(z)
    masks=[]
    for guard in guards:
        mask=0
    ...
    return dict(factors=d.tolist(),candidates=guards,selected=selected,
                certified=proven,unsafe_centers=bad,original_uncovered=original_uncovered,
                intrinsic_uncovered=intrinsic_uncovered,joint_proofs=joint_proofs,base_proofs=base_proofs,
