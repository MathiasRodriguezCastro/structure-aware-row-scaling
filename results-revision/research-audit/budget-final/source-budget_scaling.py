...
def dyadic_factor(coefficients, rhs, delta=None, epsilon=1e-6,
    ...
    target = -.5*(math.log2(lo)+math.log2(hi))
    ...
    if delta is not None:
        ...
        if rounding_allowance >= delta:
            return RowDecision(None, math.inf, float(exact_upper), math.nan,
    ...
    candidates = {min(k_high, max(k_low, math.floor(target))),
                  min(k_high, max(k_low, math.ceil(target)))}
    # Prefer the larger factor in an objective tie (the tighter residual bound).
    k = min(candidates, key=lambda x: (abs(x-target), -x))
