...
def exact_integer(value):
    """Validate an integer value and remove float/NumPy arithmetic semantics."""
    q=rational(value)
    if q.denominator!=1:raise ValueError('finite integer value required')
    return q.numerator
...
def directions(a, caps=(1,2,3,4,8,16,32), strategy='rational'):
    ...
    # Equal-valued pivots give identical ratios. Keep both signs when present
    # because exact bounded-denominator tie breaking can depend on the sign.
    for pivot_value in sorted({v for v in a if abs(v)==maximum}):
        ratios=[F(v,pivot_value) for v in a]
        for cap in caps:
            if strategy=='rounded':
...
def compression_step(a, b, v, alpha, lo, hi):
    ...
    a=list(map(exact_integer,a));v=list(map(exact_integer,v))
    lo=list(map(exact_integer,lo));hi=list(map(exact_integer,hi))
    b=exact_integer(b);alpha=exact_integer(alpha)
    if not len(a)==len(v)==len(lo)==len(hi):
        raise ValueError('row, direction and bound lengths must agree')
...
def compress_row(a, b, lo=None, hi=None, strategy='rational', max_passes=4, tolerance=1e-6):
    a=list(a)
    lo=[0]*len(a) if lo is None else list(map(exact_integer,lo))
    hi=[1]*len(a) if hi is None else list(map(exact_integer,hi))
    if not len(a)==len(lo)==len(hi):
        raise ValueError('row and bound lengths must agree')
    if any(l>u for l,u in zip(lo,hi)):
