...
from budget_scaling import exact_row_violation
...
def transform(A,b,method,blocks,nlocal):
    ...
    for k in range(blocks):
        ...
        proof=compile_block(A[row,sl],b[row],all_guards=method=='All-elimination')
        ...
        if method=='Selected-guards':
            ...
            chosen=[g for g in proof['candidates'] if all(F(v)>0 for v in g['lambda_'])]
        elif method in {'Unsafe-clauses','All-clauses'}:
            chosen=partial_nogoods(A[row,sl],b[row],proof['unsafe_centers'],all_bad=method=='All-clauses')
...
def main():
            ...
            for method in methods:
                ...
                for presolve in [False,True]:
                    for solver in ['highs','gurobi']:
                        ...
                        elif solver=='highs':status,x,wall,nodes=solve_highs(B,rhs,c,presolve,1e-6,seed)
                        else:status,x,wall,nodes=solve_gurobi(B,rhs,c,presolve,1e-6,seed,env)
    ...
    protocol=dict(purpose='exploratory; classical aggregation/elimination controls mandatory',
                  ...
                  methods=methods,epsilon=1e-6,eta=1e-6,primary='exact feasible rounded assignment at independent DP optimum',
