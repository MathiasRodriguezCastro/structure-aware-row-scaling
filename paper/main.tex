...
guarantees. On 306 generated hydrothermal-dispatch models, holding one LP basis fixed across
the representations of each model, geometric-mean row scaling lowers its condition number on
every model, by six to eight orders of magnitude in the median of each class. A gain of that size does not identify the information
...
separates what improves from what is guaranteed. In reconstructed dispatch runs with Gurobi,
scaling used 30--32\% less solver work on the 14 SG-Ter-Mer instances that every policy
completed, while one instance timed out under every scaled policy, reversing the
failure-sensitive ordering. In an exactly enumerated stress test, plain
...
representations of one model; Online Resource 1 repeats the endpoints on the six instances
without the defect.
...
does not transfer to another family, still less to solver behaviour, and the endpoints of
...
Table~S4). Scaling therefore produced a sizeable completed-run efficiency gain in this class,
...
not among them, the reversal disappears (Online Resource 1, Table~S3). The Simple class has no uniform ordering: Flat-GM
...
accepted point. On HiGHS, Budget-only
...
is unchanged by consistent re-emission and determines the admissible factors exactly. In the
