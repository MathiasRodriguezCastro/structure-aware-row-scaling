...
Suppose the operator declares 0.1\,MW for the rows that account for generation and cap it by
demand, 0.1\,MW for the piecewise-linear production envelopes, and 1{,}000\,m\textsuperscript{3}
...
declared budget. On this model the contract does not restrict the factor at all.
...
$\epsilon=10^{-6}$ and coefficient limits $[10^{-9},10^{9}]$; the last column is
$\epsilon$ divided by the geometric-mean factor, the original-unit residual that the tolerance
allows. The last group is written pre-scaled, with a coefficient of $10^7$ on the generation
variable, so 0.1\,MW there is $10^{6}$ in the units of the row as written.}
\label{tab:application-budget}
\input{tables/application-budget}
\end{table}
...
without introducing new ones on any of them. On the dispatch model the same contract is easy to
satisfy --- every policy's factor is admissible with orders of magnitude to spare --- and its
work is in the reading of a residual, which without a stated unit says nothing. GCD division with RHS flooring verified every stress configuration, but it
...
The operational evidence comes from one application: two instance classes, one solver, one seed
and 15 fixed instances per class, all from the reconstructed campaign of
\cref{subsec:design-operational}. The stress instances are small and
