...
\newcommand{\nnz}{\operatorname{nnz}}
\hypersetup{pdftitle={Pre-Export Row Scaling for Generated Mixed-Integer Programs: Mechanism Attribution and Residual-Budget Contracts},pdfauthor={Mathias Rodr\'iguez Castro}}
\graphicspath{{figs/research/}{figs/}}
\title{Pre-Export Row Scaling for Generated Mixed-Integer Programs: Mechanism Attribution and Residual-Budget Contracts}
...
changes the numbers a solver sees. This paper asks what that change buys, and what it still
guarantees. On 306 generated hydrothermal-dispatch models, with one LP basis held fixed,
geometric-mean row scaling lowers its condition number on every model, by six to eight orders
of magnitude in the median of each class. A gain of that size does not identify the information
behind it. We show that an apparent block-metadata effect was a coupling-kernel effect:
geometric-mean centering makes the block statistic identically one, so the block-relative rule
becomes Euclidean normalization, which a control without ownership information reproduces, and
the idealized coupling problem has the closed-form optimal factor $(1+\rho^2)^{-1/2}$. Residual
budgets stated in application units, by contrast, survive consistent re-emission, and we
characterize exactly which row factors a budget and coefficient limits admit. The evidence then
separates what improves from what is guaranteed. In reconstructed dispatch runs, scaling used
30--32\% less solver work on the instances every policy completed, while one scaled-policy
timeout reversed the failure-sensitive ordering. In an exactly enumerated stress test, plain
centering lost verified optima on all three solvers tested, adding the budget to the same rule
lost none, and whether scaling helped depended on the solver. A representation can be
numerically better without being operationally safer, unless what safer means is stated in the
units of the original model.
\end{abstract}
...
orders of magnitude in the median of each class (\cref{subsec:results-fixed-basis}). A gain of this
kind leaves one question open, in two halves:
\begin{quote}
