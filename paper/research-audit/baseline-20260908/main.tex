...
\abstract[Abstract]{%
Generated mixed-integer programs (MIPs) reach solvers as flat coefficient matrices, though their
generators retain each row's role, block ownership, and coupling incidence---and solver-internal
scaling then operates on that flat representation. We study an external, equivalence-preserving
row-scaling layer that uses local rows, block scales, and coupling incidence before export,
applying geometric-mean row normalization in a local--block--coupling order and verifying every
returned solution against the original unscaled constraints. We test it on three
hydrothermal-dispatch classes, two commercial solvers, and two MIPGap tolerances, against
unscaled, Ruiz-kernel, solver-internal, and role-blind full-coverage controls, together with a
synthetic mechanism grid carrying a direct spectral audit.

On the operational instances the external layer improves the solver-facing numerical diagnostics
and, under Gurobi, often improves completed-run solver time or deterministic effort beyond the
solver's own default internal scaling, which remains enabled throughout. Because the operational
balance rows are already commensurate with the blocks they connect, the distinctive coupling stage
stays inactive, so this operational gain comes primarily from the local-row normalization---a
per-row kernel computable from the flat matrix---rather than from the coupling-specific use of
metadata. The operational comparison is in this precise sense \emph{non-identifying} for the value
of metadata: it occupies the inactive-coupling regime.

The metadata-specific advantage is isolated where the mechanism is designed to act. On a controlled
heterogeneous-block grid, under an \emph{identical} kernel and identical row coverage, the
matrix-only structure-aware rule lowers the spectral pseudo-condition number by one to two orders
of magnitude relative to a role-blind rule ($30/30$ instances, $p<10^{-8}$), a difference
attributable to scaling coupling rows relative to the connected block scales. This conditioning
gain does not translate into a universal runtime gain: the Gurobi improvements are class- and
tolerance-dependent, CPLEX shows no robust benefit, and original-scale verification remains
necessary---most pointedly for the augmented right-hand-side rule (SA-Aug), which degrades
reliability on zero-right-hand-side balance rows and is why we recommend the matrix-only rule.
External scaling, metadata-specific conditioning, and solver acceleration are three distinct
questions, and the study's design keeps them separate: the operational instances establish the
practical value of the external pass, and the heterogeneous-block grid identifies when metadata
adds a numerical capability a flat rule cannot.
}
...
\begin{itemize}
  \item An external, structure-aware preprocessing pass can add numerical and computational value
  even when the commercial solver's own scaling and presolve remain enabled: under Gurobi it
  improved solver-facing diagnostics and, in several class--tolerance cells, completed-run time or
  deterministic effort beyond the default internal pipeline.
  \item Generator metadata matters specifically when coupling rows are misaligned with
  heterogeneous block scales: under identical coverage and the same coefficient-only kernel,
  block-relative scaling reduced the spectral pseudo-condition number by one to two orders of
  magnitude as the block-scale heterogeneity increased.
  \item No activation on already balanced coupling rows is the expected and desirable behavior, not
  a method failure: on the operational instances the coupling stage stays inactive because the
  balance rows are already commensurate with the connected blocks, so the structure-aware and
  role-blind exports coincide there.
  \item Equal kernel and equal coverage are prerequisites for attributing any effect to
  metadata---otherwise a kernel or coverage difference is mistaken for a metadata effect.
  \item Prefer the matrix-only rule and re-verify every returned solution against the original
  unscaled constraints: the augmented right-hand-side rule (SA-Aug) degraded original-scale
  reliability relative to Base, and even the recommended rule must remain subject to
  original-scale verification.
  \item Measure conditioning improvement and runtime improvement separately: a better numerical
  diagnostic did not imply a faster solve, and on fast instances the preprocessing cost itself,
  not the numerics, decided which rule looked faster.
\end{itemize}
...
We answer this question with a structure-aware row-scaling framework. The method scales only
whole constraints, including their right-hand sides, so it preserves the original mixed-integer
optimization problem exactly; its novelty lies in the order and source of the scales, not in the
per-row kernel. The generator first scales local component rows, then summarizes the scale of each
locally scaled block, and finally scales the global coupling rows relative to the blocks they
connect. This third stage is the only one that consumes the block metadata, and it acts only when
the coupling rows are misaligned with heterogeneous block scales. On the operational instances that
condition is not met---the balance rows already sit at scales commensurate with the connected
blocks---so the coupling stage stays inactive by design; we exercise it on a controlled synthetic
grid built to reach the regime where it is decisive. The local--block--coupling architecture is
thus evaluated in two complementary regimes: an operational one, where the coupling stage is
inactive and a per-row local kernel carries the effect, and a synthetic one, where heterogeneous
block scales activate the metadata-specific treatment.
...
Two questions follow, and the study keeps them apart. First, does the external pass add value
beyond the solver's own internal scaling? On the operational instances the answer is favorable
under Gurobi: row scaling improves the solver-facing diagnostics and, in several class--tolerance
cells, completed-run time or deterministic effort, with the solver's scaling and presolve left
enabled. Second, is that operational gain attributable to the metadata? Here the operational
comparison is \emph{non-identifying}: the coupling stage never activates, and by
\cref{prop:attribution} an equal-kernel, equal-coverage structure-aware and role-blind export
coincide once the coupling stage is inactive, so the effect that is present is carried by the
per-row local kernel, which consumes no metadata. The operational study therefore identifies the
value of external local normalization, while the controlled synthetic study supplies the
active-regime test needed to isolate the additional metadata effect: once heterogeneous block
scales activate the coupling stage, the two policies no longer coincide. (The archived operational
campaigns further used an implementation whose role map was not exhaustive, leaving a block of
market-shortage rows unscaled; the coverage-restored control of \cref{subsec:strategy-2} removes
that confound.) Concretely, the paper contributes: (i) an equivalence-preserving, generator-level
row-scaling framework for block-structured MIPs, evaluated against the solver's own internal
scaling rather than only against an unscaled base; (ii) an attribution principle
(\cref{prop:attribution}) that, under matched kernel and coverage, localizes any metadata effect to
the coupling rows and dictates the comparison the question requires; (iii) operational evidence
that external scaling can improve Gurobi numerical and computational behavior even with internal
scaling active; (iv) a controlled heterogeneous-block experiment, under identical kernel and
coverage, demonstrating a metadata-specific spectral conditioning advantage of one to two orders of
magnitude; and (v) original-scale verification together with a Gurobi--CPLEX contrast showing that
conditioning and runtime are separate outcomes. \Cref{fig:overview} summarizes the proposed layer
at a glance.
...
The paper organizes its evidence into three strands of different strength. The algebraic strand is
exact: positive row scaling preserves the MIP. The numerical strand is empirical: structure-aware
scaling lowers the solver-facing fixed-LP diagnostics on the tested instances and, on the synthetic
grid, the spectral pseudo-condition number. The performance strand is conditional: the Gurobi
runtime gains are real but class- and tolerance-dependent and are not robustly reproduced under
CPLEX, and the controlled attribution analyses show the operational runtime effect is recovered by
the local stage alone. The path-specific diagnostic moves in the intended numerical direction, but
it is neither a pure measure of representation nor a proven cause of acceleration---which is exactly
why the paper measures numerical improvement and solver acceleration separately.
...
evidence. \Cref{sec:discussion} synthesizes that evidence around external preprocessing, when
metadata adds numerical value, and the separation of conditioning from runtime.
\Cref{sec:validity} states the scope and reproducibility implications, and
...
The results follow the hierarchy of claims. We first confirm original-scale equivalence and
report the path-specific numerical diagnostics, without treating them as pure measures of
representation. We then ask whether Gurobi shows lower matched runtime or deterministic effort
with its own internal scaling and presolve left enabled, and use CPLEX as a solver contrast for
the external-preprocessing effect.

The main pattern is favorable but bounded. Under Gurobi, the structure-aware variants lower the
reported path-specific conditioning diagnostic and, in several class--tolerance cells, reduce
completed-run time or deterministic effort beyond the default internal-scaling pipeline; under
CPLEX 22.1.2.0 the same external scalings do not produce a robust runtime gain. Numerical
improvement and algorithmic acceleration are related but not identical outcomes---a conditioning
table alone would imply too much, a timing table alone too little---so the two are reported
together and their claims kept separate.
...
erases the selective--Flat gap, while the distinctive mechanism is exercised on the synthetic grid
(\cref{subsec:synthetic-results,subsec:spectral-audit}), where heterogeneous block scales activate
it.
...
\subsection{External preprocessing beyond solver-internal scaling}
\label{subsec:disc-external}

The experiments support a bounded but genuinely positive operational claim. Structure-aware
positive row scaling lowers the reported own-incumbent path-specific fixed-LP diagnostic, and
under fixed-pool intent-to-treat \PARten{} at least one variant falls below Base in each Gurobi
class and tolerance. This is measured against a solver whose own scaling and presolve are left
enabled throughout: the scaled variants always run on top of Gurobi's default internal pipeline,
and the internal-scaling control varies those settings only for the unscaled model. The comparison
is therefore not external scaling versus a naive solver, but whether a generator can usefully alter
the solver-facing formulation before the solver's pipeline begins.

The internal-scaling control (\cref{subsec:internal-scaling}) gives a class-dependent answer to
whether the solver can reproduce the external effect on its own: a tie on Simple, a descriptive
shift favoring the external rule in SG-Ter-Mer, and a tolerance-dependent outcome in Full---no
universal dominance over the internal modes is claimed. The evidence is also solver-scoped: under
CPLEX 22.1.2.0 the same external scalings yield no robust runtime gain, and the dedicated CPLEX
Flat-control cell (\cref{subsec:stage-attribution}) shows the coverage ordering is not
solver-stable at this scale. The role-blind Flat controls (\cref{tab:flat-control}) add a
caveat within Gurobi as well: the coverage ordering depends on the endpoint, the fast-class margins
are tens of milliseconds and carry a large prototype-implementation component, and on solver time
alone only the augmented Simple contrast at $0.1\%$ survives every multiplicity adjustment, with
the matrix-only contrasts inconclusive (Tables~S9--S10, S19). SG-Ter-Mer at $0.1\%$ is the one cell
in which both strict-adjusted costs remain below Base, though the SA-Aug margin is negligible, the
comparison is descriptive, and the pattern is not replicated at $1\%$; under the mixed reading
SG-Ter-Mer is descriptively below at both tolerances
(\cref{subsec:original-scale-audit,tab:par10-readings}). The operational gain is thus real and
favorable under Gurobi, and conditional---class-, tolerance-, and solver-dependent.

\subsection{When metadata adds numerical value}
\label{subsec:disc-metadata}

The attribution principle (\cref{prop:attribution}) separates the two possible sources of any
effect. The per-row local kernel needs no metadata---it reads only each row's own
coefficients---so under a matched kernel and matched coverage the only place metadata can change the
export is the coupling treatment: scaling coupling rows relative to the connected block scales
rather than by their own raw norm. (The Ruiz baselines confirm the geometric-mean kernel is not
interchangeable with a generic max-norm equilibration, \cref{subsec:variants}, but that speaks to
the kernel, not the metadata.) The operational instances do not reach the active regime. Their
coupling rows are mainly balance equations whose incidence coefficients on the generation variables
are close to one---typically $d_i=1$, occasionally $d_i=0.5$ where the binational Salto Grande
generation enters the aggregate at half weight---so the coupling proxy stays in the near-identity
band ($\widehat\rho\approx3\times10^{-4}$ in Simple and exactly $1$ in SG-Ter-Mer and Full) and the
coupling stage does not activate. The operational activation audit (\cref{subsec:added-controls})
confirms this: full exports reduce to local-row normalization, and the real instances serve as an
inactive-regime control.

This inactivity is the intended behavior, not a shortfall: a structure-aware rule should not
intervene merely because metadata are available; it should intervene only when the structural scale
mismatch is present. The controlled heterogeneous-block grid supplies exactly that mismatch, and
there the same-kernel, same-coverage policies separate cleanly. The matrix-only block-relative rule
lowers the spectral pseudo-condition number by about one order of magnitude at exponent range
$[-2,2]$ and two orders at $[-3,3]$ ($30/30$ instances, $p<10^{-8}$; \cref{tab:metadata-grid}), the
advantage growing with heterogeneity and negligible when the blocks are near-homogeneous---the
regime the operational rows occupy. Because kernel and coverage are held fixed, this growth is
attributable to the block-relative use of metadata alone. The same grid bounds the claim: on the
global coefficient-and-right-hand-side magnitude-range proxy a full-coverage flat kernel matches or
beats every structure-aware variant, the oracle-like behavior expected of complete-row multiplier
injections when no safeguard binds, so the metadata advantage is specific to spectral
pseudo-conditioning under heterogeneous coupling, not a general dominance on every diagnostic. With
the reliability floor in place, original-scale feasibility and objective agreement hold across the
grid; the severe heterogeneous cells are also where, with the floor disabled, the common-factor
variants expose their reliability failure mode (\cref{subsec:synthetic-results}).

The operational coupling null must therefore be read as a property of the tested formulation, not as
evidence against the coupling mechanism. It reflects this dispatch model's lack of a transmission
network. A model with DC power-flow constraints---the standard next step, present in most
operational tools---would add coupling rows mixing per-unit susceptances of order
$10^{2}$--$10^{3}$, phase angles of order $1$, and flow limits of order $10^{2}$--$10^{3}$ against
blocks scaled in MW and $\mathrm{hm}^3$; inter-area reserve sharing and interconnection limits do
the same. In any of these $\widehat\rho$ would plausibly leave the near-unity regime and the stage
would activate. The null reported here is thus evidence about dispatch formulations whose coupling
rows are near-unity balances, and extending the study to network- or reserve-coupled models is the
step that would most raise the ceiling of this line of work.

\subsection{Conditioning versus runtime}
\label{subsec:disc-conditioning}

Better conditioning is a numerical result, not a theorem of faster branch-and-bound. If it drove
search speed, the best-conditioned variant would be the fastest, but the data do not line up that
way: the augmented rule improves solver-level conditioning more than the matrix-only rule, yet the
matrix-only rule wins runtime more often in Simple and SG-Ter-Mer, and the association between the
conditioning ratio and speedup is weak (a small Spearman correlation explaining a few percent of
variance). A commercial MIP run is not a linear-algebra experiment: presolve reductions, relaxation
bases, cuts, incumbents, branching, heuristics, seeds, and tolerances all stand between the exported
matrix and the runtime, so external scaling can improve one stage while the binding bottleneck lies
elsewhere. This has precedent---large-scale LP experiments find the runtime effect of scaling
irregular even where conditioning improves \cite{elble2012scaling}, and the conditioning of the LP
relaxations evolves along the branch-and-cut tree in ways a static snapshot cannot summarize
\cite{miltenberger2018numerics}. We therefore treat the own-incumbent fixed-LP quantity as a pooled
description of solver paths---a representation-plus-path diagnostic, since it compares different
integer solutions whenever two runs find different incumbents (\cref{tab:flat-control})---not as a
pure measure of representation or a proven cause of speedup.

The CPLEX evidence sharpens the same separation: several transformations improve static or
solver-level diagnostics, yet paired CPLEX runtime stays neutral to unfavorable. This does not
weaken the numerical claim; it limits the performance claim. We probed the most natural
mechanism---that CPLEX presolve neutralizes the external scaling---by re-running Simple under CPLEX
with presolve disabled; the per-instance effect stayed essentially neutral (median paired speedup
$\approx1.00$, structure-aware faster on about half of the $204$ instances), so presolve does not
explain the Simple-class neutrality, while no no-presolve run is available for SG-Ter-Mer or Full.
The methodological lesson is that numerical preprocessing for MIPs must be judged on four separable
layers---algebraic equivalence, numerical diagnostics, matched runtime, and solver contrast---and
never on one coefficient-range statistic or one runtime table; this study reports all four and keeps
their claims separate.

\subsection{Reliability and the recommended variant}
\label{subsec:disc-reliability}

The augmented rule (SA-Aug) folds the right-hand side into the per-row and per-block extremes, and
that is where reliability is at risk. Under the pre-declared absolute criterion SA-Aug degrades
original-scale reliability relative to Base, with failures concentrated on zero-right-hand-side
balance rows, where a large $|b_r|$ plays no stabilizing role yet still enters the extremes. The
clean evidence that the right-hand side is largely unnecessary comes from the local-only ablation
pair, whose members differ \emph{only} in whether $b$ enters the per-row extremes: the
coefficient-only version stays within $2$--$3\%$ in the two well-populated classes ($2.06$ vs
$2.01$\,s in Simple, $8.83$ vs $8.58$\,s in SG-Ter-Mer) and recovers part of the difference in Full,
where the augmented rule is nominally about $11\%$ faster ($78.9$ vs $70.9$\,s). That Full figure
rests on one run per cell in a class with three or four timeouts per variant, so its two geometric
means are taken over different solved subsets and are descriptive only (Table~S8); we do not read it
as a residual advantage. The head-to-head \methodAug{}--\methodMat{} comparison is consistent but
confounds the right-hand side with the coupling criterion (\cref{subsec:strategy-2}), so we do not
read it as an ablation. The matrix-only rule therefore has both the cleaner numerical
rationale---it never lets a right-hand-side magnitude drive a coefficient scale---and the cleaner
reliability profile, and it is the variant we recommend. In every case the returned solution must be
verified on the original unscaled constraints, and runtime must be treated as a separate empirical
criterion at deployment.
...
An incomplete role map in the tested implementation is the scope limitation that most shapes how the
coverage results should be read. That implementation did not realize an exhaustive map
...
category and each such row is generated by the market/shortage module. The selective-versus-Flat
comparison therefore confounds structural treatment with unequal row coverage, and its SG-Ter-Mer
difference reflects that coverage gap rather than the metadata. The coverage-restored full-coverage
rule removes the confound---once every row is covered the selective--Flat difference disappears
(\cref{subsec:added-controls})---but it equalizes only the \emph{set} of scaled rows and is not
equivalent to reconstructing a native exhaustive block assignment inside the generator, and the
attribution principle (\cref{prop:attribution}) makes the coverage-matched comparison explicit. A
native per-block assignment of every generated row, together with operational instances that drive
the coupling stage into its active regime, remains the natural next step.
...
choose positive, equivalence-preserving row scalings actually helps. Four questions organize the
answer.

\emph{Does external scaling help the operational application?} Under Gurobi, yes: external
generator-level row scaling improves the solver-facing numerical diagnostics and, in several
class--tolerance cells, completed-run time or deterministic effort beyond the default
internal-scaling pipeline, which stays enabled throughout. The result is class- and
tolerance-dependent and is not robustly reproduced under CPLEX 22.1.2.0.

\emph{Is that operational runtime gain attributable to the metadata?} Not to the distinctive
coupling mechanism. The operational coupling rows are already balanced, so by
\cref{prop:attribution} the structure-aware and role-blind matrix-only exports coincide
byte-for-byte there, and the measured effect comes primarily from local-row normalization---a
per-row kernel that consumes no metadata. The operational comparison is in this precise sense
non-identifying for the value of metadata, not a refutation of it.

\emph{Does the metadata itself provide an additional numerical advantage?} Yes, when the blocks
connected by the coupling rows occupy heterogeneous scales. Under an identical kernel and identical
coverage, a controlled heterogeneous-block experiment isolates a spectral pseudo-conditioning
improvement of one to two orders of magnitude for the matrix-only rule over a role-blind rule
(\cref{tab:metadata-grid}), attributable to block metadata alone, while the augmented collective
rule obtains no such advantage.

\emph{Does better conditioning guarantee a faster MIP solution?} No. Conditioning is a distinct
numerical contribution whose runtime consequence must be evaluated separately for each solver
pipeline; establishing this bounded reading required separating four kinds of evidence---algebraic
equivalence, original-scale feasibility, solver-facing diagnostics, and matched per-solver runtime.
For deployment the preferred rule is a coverage-safeguarded matrix-only scaling: cover every row
...
that fails original-scale verification---in preference to the augmented rule, whose use of the
right-hand side adds a documented reliability failure mode.

External scaling, metadata-specific conditioning, and solver acceleration are thus three distinct
layers of evidence, and the study's design keeps them apart. The operational instances establish the
practical value of the external pass under Gurobi; the controlled heterogeneous-block study
identifies when metadata adds a numerical capability a flat rule cannot supply. The two experiments
are complementary---one locates the value of external local normalization in a realistic pipeline,
the other isolates the structural metadata mechanism---and together they answer, for generated
block-structured MIPs, both whether generator-level preprocessing helps and when block metadata
