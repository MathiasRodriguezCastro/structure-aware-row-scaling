...
def main():
    ...
    for item in design['selected']:
        ...
        # Binding properties return complete Python lists. Cache once rather
        # than copying both vectors at every row of a large model.
        row_upper=lp.row_upper_;row_lower=lp.row_lower_
        ...
        for i in range(lp.num_row_):
            ...
            for sign,rhs in [(1,row_upper[i]),(-1,-row_lower[i])]:
                ...
                certificates[name+f'_row{i}_sign{sign}']=dict(proof=proof,original_score=str(original_score),
                       ...
                       simple_a=list(map(str,sr)),simple_b=str(sb))
