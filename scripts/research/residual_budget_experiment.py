...
def solve_cplex(A, b, profits, presolve, tolerance, seed):
    import cplex
    m = cplex.Cplex()
    for stream in (m.set_log_stream, m.set_results_stream, m.set_warning_stream, m.set_error_stream):
        stream(None)
    p = m.parameters
    p.threads.set(1)
    p.randomseed.set(seed)
    p.preprocessing.presolve.set(1 if presolve else 0)
    p.timelimit.set(10.)
    p.mip.tolerances.mipgap.set(0.)
    p.mip.tolerances.absmipgap.set(0.)
    p.simplex.tolerances.feasibility.set(tolerance)
    p.mip.tolerances.integrality.set(tolerance)
    n = A.shape[1]
    m.objective.set_sense(m.objective.sense.minimize)
    m.variables.add(obj=(-profits).tolist(), lb=[0.]*n, ub=[1.]*n, types='B'*n)
    rows = [cplex.SparsePair(ind=[int(j) for j in np.flatnonzero(r)], val=[float(v) for v in r[r != 0]])
            for r in A]
    m.linear_constraints.add(lin_expr=rows, senses='L'*len(rows), rhs=[float(v) for v in b])
    tic = time.perf_counter()
    m.solve()
    elapsed = time.perf_counter()-tic
    sol = m.solution
    x = np.array(sol.get_values()) if sol.is_primal_feasible() else None
    result = (sol.get_status_string(), x, elapsed, sol.progress.get_num_nodes_processed())
    m.end()
    return result
...
def main():
    ...
    ap.add_argument('--solvers', nargs='+', choices=['highs', 'gurobi', 'cplex'], default=['highs', 'gurobi'])
    ap.add_argument('--models-from', type=Path, help='load the stored models (checked against regeneration)')
    args = ap.parse_args()
    ...
    if 'cplex' in args.solvers:
        import cplex
        versions['cplex'] = cplex.Cplex().get_version()
    ...
    for family in FAMILIES:
        for seed in protocol['seeds']:
            for exponent in protocol['exponents']:
                ...
                if args.models_from:
                    stored = np.load(args.models_from/f'{model_id}.npz')
                    for key, value in dict(A=A, b=b, c=c, delta=delta).items():
                        assert np.array_equal(stored[key], value), (model_id, key)
                    assert int(stored['optimum']) == optimum
                    ...
                    for method in order:
                        ...
                        for solver in args.solvers:
                            ...
                            if d is None:
                                ...
                                status, x, wall, nodes = solve_cplex(d[:, None]*A, d*b, c, presolve, tol, seed)
                            else:
